...
OPTIONAL_DATA_PATHS = {
    ...
    "/Bms/Charge/AllowedA": lambda value: value,
    "/Bms/Discharge/AllowedA": lambda value: value,
    "/Dc/0/MaxCellVoltage": lambda value: value,
    "/Dc/0/MinCellVoltage": lambda value: value,
    "/Dc/0/AlarmFlags": lambda value: value,
    "/History/ChargeCycles": lambda value: value,
}
...
class VirtualInvertedBattery:
    def __init__(self):
        ...
        self.active_paths: Dict[str, callable] = {}
        ...
        logger.info("Started monitoring %s with %d paths.", SOURCE_SERVICE, len(self.active_paths))
    ...
    def _setup_service_paths(self):
        ...
        self.dbusservice.add_path("/Mgmt/Connection", "Virtual Battery")
        ...
        self.active_paths = dict(CORE_DATA_PATHS)
        for path in CORE_DATA_PATHS:
        ...
        for path, transform in OPTIONAL_DATA_PATHS.items():
            try:
                ...
                try:
                    ...
                    self.dbusservice.add_path(path, 0)
                    logger.info("Optional path available: %s", path)
                except:
                    pass
            except:
                logger.debug("Optional path not available: %s", path)
